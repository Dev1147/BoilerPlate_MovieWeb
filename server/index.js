...
app.get('/api/users/auth', auth,(req,res)=>{
  ...
  console.log("인증 미들웨어 실행됨, req.user:", req);
  
  if (!req.user) {
    return res.status(401).json({ isAuth: false, message: "Unauthorized. Please log in." });
  }
  // console.log('_id 정보',req.user._id)
  // console.log('role 정보',req.user.role)
  // console.log('email 정보',req.user.email)
  // console.log('name 정보',req.user.name)
  // console.log('lastname 정보',req.user.lastname)
  // console.log('role 정보',req.user.role)
  // console.log('image 정보',req.user.image)
...
// 로그아웃
app.get('/api/users/logout', auth, (req, res) => {
  //console.log('로그아웃 유저 확인:', req.user); 
